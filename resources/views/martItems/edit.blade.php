...
@section('content')
    <div class="page-wrapper">
        ...
        <div class="container-fluid">
            ...
            <div class="card-body">
                    <h4 class="card-title mb-4">Edit Mart Item</h4>
                    ...
                    <form method="POST" action="{{ route('mart-items.update', $item->id) }}" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" name="vendorID" id="vendorID" required>
                                ...
                                @foreach($vendors as $vendor)
                                    <option value="{{ $vendor->id }}" {{ old('vendorID', $item->vendorID) == $vendor->id ? 'selected' : '' }}>
                                        {{ $vendor->title }}
                                    </option>
                                @endforeach
                                    </select>
                        ...
                        <div class="form-row">
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <input type="number" step="0.01" min="0" class="form-control" id="disPrice" name="disPrice" value="{{ old('disPrice', $item->disPrice) }}">
                            </div>
                                </div>
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <select class="form-control" id="categoryID" name="categoryID" required>
                                    ...
                                    @foreach($categories as $category)
                                        <option value="{{ $category->id }}" data-section="{{ $category->section }}" {{ old('categoryID', $item->categoryID) == $category->id ? 'selected' : '' }}>
                                            {{ $category->title }}
                                        </option>
                                    @endforeach
                                        </select>
                                ...
                                <select class="form-control" id="subcategoryID" name="subcategoryID">
                                    ...
                                    @foreach($subcategories as $subcategory)
                                        <option value="{{ $subcategory->id }}"
                                                ...
                                                {{ old('subcategoryID', $item->subcategoryID) == $subcategory->id ? 'selected' : '' }}>
                                            {{ $subcategory->title }}
                                        </option>
                                    @endforeach
                        </select>
                        ...
                        <div class="form-row">
                            <div class="form-group col-md-6">
                                ...
                                <select class="form-control" id="brandID" name="brandID">
                                    ...
                                    @foreach($brands as $brand)
                                        <option value="{{ $brand->id }}" {{ old('brandID', $item->brandID) == $brand->id ? 'selected' : '' }}>
                                            {{ $brand->name }}
                                        </option>
                                    @endforeach
                        </select>
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <input type="text" class="form-control" id="section" name="section" value="{{ old('section', $item->section) }}" readonly>
                </div>
            </div>
                        ...
                        <div class="form-row">
                            <div class="form-group col-md-6">
                                ...
                                <input type="number" class="form-control" id="quantity" name="quantity" min="-1" value="{{ old('quantity', $item->quantity) }}">
                    </div>
                            <div class="form-group col-md-6">
                                ...
                                @if ($item->photo)
                                    <div class="mt-2">
                                        <img src="{{ $item->photo }}" alt="item image" style="height:80px" class="rounded">
                        ...
                        <div class="form-row">
                            <div class="form-group col-md-3">
                                ...
                                <input type="number" class="form-control" id="calories" name="calories" min="0" value="{{ old('calories', $item->calories) }}">
                    </div>
                            <div class="form-group col-md-3">
                                <label for="grams">Grams</label>
                                <input type="number" class="form-control" id="grams" name="grams" min="0" value="{{ old('grams', $item->grams) }}">
                </div>
                            <div class="form-group col-md-3">
                                <label for="proteins">Proteins</label>
                                <input type="number" class="form-control" id="proteins" name="proteins" min="0" value="{{ old('proteins', $item->proteins) }}">
                    </div>
                            <div class="form-group col-md-3">
                                <label for="fats">Fats</label>
                                <input type="number" class="form-control" id="fats" name="fats" min="0" value="{{ old('fats', $item->fats) }}">
                </div>
            </div>

                    <div class="form-group">
                            <label for="description">Description <span class="text-danger">*</span></label>
                            <textarea class="form-control" id="description" name="description" rows="5" required>{{ old('description', $item->description) }}</textarea>
                </div>
            ...
            <div class="row">
                                ...
                                @foreach($featureFields as $field => $label)
                                    <div class="col-md-3">
                                        <div class="form-check">
                                            <input type="checkbox" class="form-check-input" id="{{ $field }}" name="{{ $field }}" value="1" {{ old($field, $item->$field) ? 'checked' : '' }}>
                    ...
                    <div class="form-group">
                            <div class="form-check form-check-inline">
                                <input type="checkbox" class="form-check-input" id="publish" name="publish" value="1" {{ old('publish', $item->publish) ? 'checked' : '' }}>
                                <label class="form-check-label" for="publish">Publish</label>
                    </div>
                            <div class="form-check form-check-inline">
                                <input type="checkbox" class="form-check-input" id="isAvailable" name="isAvailable" value="1" {{ old('isAvailable', $item->isAvailable) ? 'checked' : '' }}>
                                <label class="form-check-label" for="isAvailable">Available</label>
                </div>
                            <div class="form-check form-check-inline">
                                <input type="checkbox" class="form-check-input" id="nonveg" name="nonveg" value="1" {{ old('nonveg', $item->nonveg) ? 'checked' : '' }}>
